...
import { useCreateRestaurantLocationMutation } from '@/services/restaurantLocationApi'
...
const GeneralInformationCard: React.FC<ControlType> = ({ control, currentImage, onFileChange }) => {
  return (
    <Card>
      <CardHeader>
        <CardTitle as={'h4'}>Restaurant Location Add</CardTitle>
      </CardHeader>
      <CardBody>
        <Row>
          <Col lg={6}>
...
const RestaurantsLocationAdd: React.FC = () => {
  ...
  const [createRestaurantLocation, { isLoading }] = useCreateRestaurantLocationMutation()
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await createRestaurantLocation({ name: data.name, address: data.address, image }).unwrap()
      alert('Restaurant location created successfully')
      push('/pages/restaurants-location')
    } catch (e: any) {
      alert(e?.data?.message || e?.message || 'Failed to create restaurant location')
    }
  }
